...
#include<stdlib.h>
int line=0;
int get_substr(FILE *fp,char *sub_str[],long int *indx);
//void search_in_file(FILE *fp,char *sub_str[],int substr_count,int indx_correction);
	...
	while( (ch=getc(fp))!=EOF )
	{
		...
		for(j=0;j<substr_count;j++)
		{
			...
			if(substring_count>0)
				printf("\nsubstring %s is present at index ",sub_str[j]);
